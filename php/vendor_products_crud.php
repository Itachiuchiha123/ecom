...
switch ($action) {
    case 'create':
        ...
        $stmt = $conn->prepare("INSERT INTO products (name, description, price, category_id, vendor_id, rating, image) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssdidds", $name, $description, $price, $category_id, $vendor_id, $rating, $image);
    ...
    case 'update':
        ...
        $verify_stmt = $conn->prepare("SELECT id FROM products WHERE id = ? AND vendor_id = ?");
        $verify_stmt->bind_param("ii", $product_id, $vendor_id);
        $verify_stmt->execute();
        if ($verify_stmt->get_result()->num_rows === 0) {
            echo json_encode(['success' => false, 'message' => 'You do not have permission to edit this product']);
            exit;
        }
        ...
        $stmt = $conn->prepare("UPDATE products SET name = ?, description = ?, price = ?, category_id = ?, rating = ?, image = ? WHERE id = ? AND vendor_id = ?");
        $stmt->bind_param("ssdidsii", $name, $description, $price, $category_id, $rating, $image, $product_id, $vendor_id);
    ...
    case 'delete':
        ...
        // Verify product belongs to this vendor
        $verify_stmt = $conn->prepare("SELECT id, image FROM products WHERE id = ? AND vendor_id = ?");
        $verify_stmt->bind_param("ii", $product_id, $vendor_id);
        $verify_stmt->execute();
        $product = $verify_stmt->get_result()->fetch_assoc();
